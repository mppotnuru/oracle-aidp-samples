  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        notebookFolder: { type: 'string', description: 'Optional target folder. Defaults to /Workspace/aidp_claude_notebook_workflow_<runId>.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        config: { $ref: '#/$defs/config' },
        runId: { type: 'string', description: 'Optional stable run ID. Defaults to UTC timestamp.' },
        notebookFolder: { type: 'string', description: 'Optional target folder. Defaults to /Workspace/aidp_claude_three_notebook_<runId>.' },
        jobPath: { type: 'string', default: '/Workspace/jobs' },
        clusterName: { type: 'string', description: 'Cluster display name. Defaults to AIDP_CLUSTER_NAME or cluster key.' },
        evidenceDir: { type: 'string', description: 'Local evidence directory. Defaults to ./qa-runs/<runId>/ask-aidp-plugin.' },
        pollIntervalSeconds: { type: 'integer', minimum: 1, maximum: 300, default: 20 },
...
async function notebookWorkflow(input) {
  ...
  const notebookFolder = input.notebookFolder || `/Workspace/aidp_claude_notebook_workflow_${runId}`;
  ...
  const createJobBody = {
    name: `aidp_claude_notebook_workflow_${notebooks.length}_${runId}.job`,
    description: `Claude Ask AIDP plugin workflow that runs ${notebooks.length} notebooks in sequence.`,
    path: jobPath,
    maxConcurrentRuns: 1,
...
function buildWorkflowNotebookSpecs(input, runId) {
  if (Array.isArray(input.notebooks) && input.notebooks.length) {
    return input.notebooks.map((notebook, index) => {
      const stage = index + 1;
      const taskKey = notebook.taskKey || `claude_notebook_${stage}`;
      const previousTaskKey = index === 0 ? null : (input.notebooks[index - 1].taskKey || `claude_notebook_${index}`);
      return {
        stage,
  ...
  return Array.from({ length: count }, (_, index) => {
    const stage = index + 1;
    const taskKey = `claude_stage_${stage}`;
    return {
      stage,
      name: `${String(stage).padStart(2, '0')}_stage_${stage}.ipynb`,
      taskKey,
      dependsOn: index === 0 ? [] : [{ taskKey: `claude_stage_${stage - 1}` }],
      source: ''
    };
  });
}
...
function generatedNotebookSource(stage, taskKey) {
  return [
    ...
    `stage = ${stage}`,
    `task_key = "${taskKey}"`,
    'print(f"AIDP Claude plugin workflow stage {stage} task {task_key} starting")',
    '',
    ...
    'assert total >= stage',
    `print(f"AIDP_CLAUDE_STAGE_${stage}_SUCCESS total={total}")`
  ].join('\n');
}

function notebookContent(stage, source, taskKey = '') {
  const cells = source
    ? customNotebookCells(stage, source, taskKey || `claude_stage_${stage}`)
    : generatedNotebookCells(stage, taskKey || `claude_stage_${stage}`);
  return {
    nbformat: 4,
    nbformat_minor: 5,
    metadata: {
...
function customNotebookCells(stage, source, taskKey) {
  return [
    ...
    notebookCodeCell([
      ...
      `print("AIDP_CLAUDE_STAGE_${stage}_CUSTOM_SOURCE_COMPLETE")`
    ].join('\n'))
  ];
}
